...
export default function TarefasProfessor() {
  ...
  const handleDeleteComment = async (comentarioId) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8000/comentarios/${comentarioId}`, {
        method: "DELETE",
        headers: {
      ...
      if (res.ok || res.status === 204) {
        ...
        setSelectedTask(prevTask => ({
          ...prevTask,
          comentarios: prevTask.comentarios.filter(c => c.id !== comentarioId)
        }));
        setTasks(prevTasks =>
          prevTasks.map(task =>
            task.id === selectedTask.id
              ? {
                  ...task,
                  comentarios: task.comentarios.filter(c => c.id !== comentarioId)
                }
              : task
          )
        );
        ...
        toast.error(errorData.detail || "Erro ao excluir comentário");
      }
    } catch (error) {
  ...
  const handleEditComment = async (e) => {
    ...
    const res = await fetch(
      `http://localhost:8000/comentarios/${editingCommentId}`,
      {
        method: "PUT",
        headers: {
    ...
    if (res.ok) {
      ...
      // Atualizar comentário editado localmente
      setSelectedTask(prevTask => ({
        ...prevTask,
        comentarios: prevTask.comentarios.map(c =>
  ...
  const handleSelecionarAluno = async (aluno) => {
    ...
    if (tccIdAluno) {
      ...
      const token = localStorage.getItem("token");
      if (!tccIdAluno) {
        toast.error("ID do TCC não encontrado");
        return;
      }
      ...
      if (tarefasRes.ok) {
        ...
        // Se vier erro do backend, não renderiza como tarefa
        if (Array.isArray(tarefas)) {
          setTasks(tarefas);
        } else {
          setTasks([]);
          toast.error("Erro ao carregar tarefas: " + (tarefas.detail || "Dados inválidos."));
        }
      } else {
        setTasks([]);
        toast.error("Erro ao carregar tarefas do aluno.");
      }
    } else {
      setTasks([]);
    }
  ...
  const onDragEnd = async (result) => {
    ...
    if (source.droppableId !== destination.droppableId) {
      ...
      const token = localStorage.getItem("token");
      try {
        const response = await fetch(`http://localhost:8000/tarefas/${movedTask.id}/status`, {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleAddTask = async (e) => {
    ...
    const token = localStorage.getItem("token");
    // Send FormData, since backend expects FormData
    ...
    try {
      setLoading(true);
      if (!tccId) {
        setAddTaskError("ID do TCC não encontrado");
        return;
      }
    ...
    try {
      ...
      if (response.ok || response.status === 204) {
        ...
        if (alunoSelecionado && tccId) {
          ...
          const tarefasRes = await fetch(`http://localhost:8000/tccs/${tccId}/tarefas`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (tarefasRes.ok) {
            const tarefas = await tarefasRes.json();
  ...
  const handleSaveEditTask = async (id) => {
    const token = localStorage.getItem("token");
    // Send JSON, not FormData, to match backend expectations
    const tarefaUpdate = {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/tarefas/${id}`, {
        ...
        body: JSON.stringify(tarefaUpdate),
      });
      if (response.ok) {
        const tarefaAtualizada = await response.json();
  ...
  async function handleAddComentario(e) {
    e.preventDefault();
    if (!novoComentario.trim() || !selectedTask) {
    ...
    try {
      ...
      if (res.ok) {
        const newComment = {
          ...data,
          usuario_nome: data.usuario?.nome || "Usuário" // Fallback para o nome do usuário
        };
        
        // Atualizar diretamente o selectedTask com o novo comentário
        setSelectedTask(prevTask => ({
          ...prevTask,
          comentarios: [...(prevTask.comentarios || []), newComment]
        }));

        // Atualizar também a lista de tarefas
        setTasks(prevTasks => 
          prevTasks.map(task => 
            task.id === selectedTask.id
              ? {
        ...
        toast.error(data.detail || "Erro ao adicionar comentário");
      }
    } catch (error) {
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-green-50 to-white">
          ...
          <div>
            ...
            <Modal open={editTaskId !== null} onClose={() => setEditTaskId(null)}>
              ...
              >
                ...
                {/* Comentários - design antigo */}
                {editTaskId && tasks.find(t => t.id === editTaskId)?.comentarios && (
                  <div className="mt-6">
                    <h4 className="font-bold text-[#2F9E41] mb-2">Comentários</h4>
                    <div className="space-y-3 mb-4">
                      {tasks.find(t => t.id === editTaskId).comentarios.length > 0 ? (
                        tasks.find(t => t.id === editTaskId).comentarios.map((comentario) => (
                          <div key={comentario.id} className="bg-gray-100 rounded p-2">
                            {editingCommentId === comentario.id ? (
                              <form onSubmit={handleEditComment} className="flex gap-2">
                                ...
                                <button
                                  type="submit"
                                  className="bg-[#2F9E41] text-white px-3 py-1 rounded hover:bg-[#217a32] text-sm"
                                >
                                  Salvar
                                </button>
                                <button
                                ...
                                <div className="flex items-center justify-between mt-1">
                                  ...
                                  <div className="flex gap-2">
                                    <button
                                      ...
                                      className="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600 text-xs font-semibold"
                                    >
                                      Editar
                                    </button>
                                    <button
                                      onClick={() => handleDeleteComment(comentario.id)}
                                      className="bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600 text-xs font-semibold"
                                    >
                                      Excluir
                                    </button>
                                  </div>
                        ...
                        <div className="text-gray-400 text-center text-sm">Nenhum comentário</div>
                      )}
                    </div>
                    <form onSubmit={handleAddComentario} className="flex gap-2 mt-2">
                      <input
                        type="text"
                        className="flex-1 border-2 border-[#2F9E41] rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-[#2F9E41]"
                      ...
                      <button
                        type="submit"
                        className="bg-[#2F9E41] text-white px-4 py-1 rounded-lg hover:bg-[#217a32] transition font-semibold"
                      >
                        Adicionar
                      </button>
                    </form>
                    ...
                    <Droppable droppableId={col.key} key={col.key}>
                      {(provided, snapshot) => (
                        <div
                          ...
                          className={`bg-white rounded-2xl shadow-xl min-w-[270px] max-w-[400px] flex flex-col transition h-full border-2 ${snapshot.isDraggingOver ? "bg-green-50 border-[#2F9E41]" : "border-gray-200"}`}
                        >
                          <div
                                ...
                                {(provided, snapshot) => (
                                  <div key={task.id}>
                                    <div
                                      ...
                                      className={`bg-gray-100 rounded-xl p-3 shadow flex flex-col gap-2 relative border ${snapshot.isDragging ? "ring-2 ring-green-400 border-[#2F9E41]" : "border-gray-200"} hover:bg-green-50 transition`}
                                    ...
                                    >
                                      <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-1">
                                          <button
                                            className="p-1 rounded hover:bg-gray-200 text-[#2F9E41]"
                                            onClick={(e) => {
                                              e.stopPropagation();
                                          ...
                                          >
                                            ...
                                            {task.comentarios?.length > 0 && (
                                              <span className="absolute -top-1 -right-1 bg-[#2F9E41] text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">
      ...
      {/* Modal de comentários: abre ao clicar no ícone de comentários */}
      <Modal open={showTaskModal} onClose={() => setShowTaskModal(false)}>
        {selectedTask && (
          <div>
            <h3 className="text-xl font-bold text-[#2F9E41] mb-2">Comentários da Tarefa</h3>
            <div className="space-y-3 mb-4">
